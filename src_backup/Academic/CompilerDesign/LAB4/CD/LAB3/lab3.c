   ...
   for(int i=0;i<grammar.rule_count;i++)
   {
       for(int j=0;j<grammar.rules[i].rhs_count;j++)
       {
           ...
           if(strcspn(temp,s)>=0 && strcspn(temp,s)<temp_len-1)
           {
               ...
               // Start from the position after the symbol
               int pos = strcspn(temp,s) + 1;
               
               // Look at all remaining symbols in the production
               while(pos < temp_len)
               {
                   char first[100];
                   ...
                   // Add non-epsilon symbols to follow
                   ...
                   // If current symbol doesn't have epsilon, we're done
                   ...
                   // If we've reached the end and all symbols had epsilon
                   if(pos >= temp_len)
                   {
...
int** parsingTable(Grammar grammar,char* Terminals)
{
    int nonTerminalCount = grammar.rule_count;
    int terminalCount = strlen(Terminals);

    int **parsingTable=(int**)malloc(grammar.rule_count*sizeof(int*));
    if (parsingTable == NULL) {
        return NULL; 
    }
    
    for(int i = 0; i < nonTerminalCount; i++) {
        parsingTable[i] = (int*)malloc((terminalCount + 1) * sizeof(int));
        if (parsingTable[i] == NULL) {
            for (int j = 0; j < i; j++) {
                free(parsingTable[j]);
            }
            free(parsingTable);
...
int main(int argc,char **argv)
{
   ...
   int** parsingtable=parsingTable(grammar,Terminals);
