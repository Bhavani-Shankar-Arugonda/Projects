...
void addOldProd(Item *a, Item *b, char c, int sourceItem)
{
    b->count = 0;
    b->transition = sourceItem;  
    b->sym = c;
    b->already = 0;  

    for (int i = 0; i < a->count; i++)
    {
        char *prodStr = a->prod[i];
        ...
        if (dotPos != -1 && dotPos + 1 < len && prodStr[dotPos + 1] == c)
        {
            char arr[max] = {0};
            int k = 0;
...
void computeClosure(Item* a)
{
    ...
    do 
	{
        ...
        for(int i = 0; i < a->count; i++)
        {
            ...
            if(dotPos != -1 && a->prod[i][dotPos + 1] != '\0' && isNt(a->prod[i][dotPos + 1]))
            {
                char nextNt = a->prod[i][dotPos + 1];
    ...
    while (k < item_count)
    {
        ...
        for (int i = 0; i < a->count; i++)
        {
            for (int j = 0; a->prod[i][j] != '\0'; j++)
            {
                ...
                {
                    char nextSym = a->prod[i][j + 1];
                    
                    int found = 0;
                    for(int m = 0; m < symCount; m++)
                    {
                        if(symbols[m] == nextSym)
                        {
                            found = 1;
                            break;
                        }
...
int main(int argc, char *argv[])
{
	...
	{
		printf("I%d", i);
		if(items[i].transition != -1)
		{
			printf(" : goto (I%d, %c)\n",  items[i].transition, items[i].sym);
		}
		else
		{
			printf("\n");
