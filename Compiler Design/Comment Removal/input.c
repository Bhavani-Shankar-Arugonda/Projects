...
// Find index of a non-terminal in grammar rules
...
// Get next available non-terminal for new productions
char getNewNonTerminal(Grammar* grammar) {
    bool used[26] = {false};
    
    // Mark all used non-terminals
    for(int i = 0; i < grammar->rule_count; i++) {
    ...
    // Find first unused non-terminal
    for(int i = 0; i < 26; i++) {
        if(!used[i]) {
            return 'A' + i;
        }
...
// Remove direct left recursion for a specific non-terminal
void removeDirectLeftRecursion(Grammar* grammar, int rule_index) {
    ...
    // Separate alpha (left recursive) and beta (non-left recursive) productions
    for(int i = 0; i < rule->rhs_count; i++) {
        if(rule->rhs[i][0] == A) {
            // Left recursive production A -> A alpha
            strcpy(alpha[rule_index][alpha_count], rule->rhs[i] + 1); // Remove A from beginning
            alpha_count++;
        } else {
            // Non-left recursive production A -> beta
    ...
    if(alpha_count > 0) {
        // Left recursion found, need to eliminate it
        char A_prime = getNewNonTerminal(grammar);
        
        // Clear current productions for A
        rule->rhs_count = 0;
        
        // A -> beta A' for all beta productions
        ...
        // Create new production A' -> alpha A' | ε
        Production new_rule;
        new_rule.lhs = A_prime;
        new_rule.rhs_count = 0;
        
        // A' -> alpha A' for all alpha productions
...
// Substitute productions of Aj into Ai
void substituteProductions(Grammar* grammar, int i, int j) {
    ...
    for(int k = 0; k < rule_i->rhs_count; k++) {
        if(rule_i->rhs[k][0] == Aj) {
            ...
            strcpy(remaining, rule_i->rhs[k] + 1); // Get part after Aj
            ...
            // Keep this production as is
            strcpy(temp_productions[temp_count], rule_i->rhs[k]);
            temp_count++;
        }
    }
    
    // Update rule_i with new productions
...
void removeIndirectLeftRecursion(Grammar* grammar) {
    ...
    for(int i = 0; i < grammar->rule_count; i++) {
        for(int j = 0; j < i; j++) {
            // For each Ai -> Aj γ where j < i, substitute Aj productions
            char Ai = grammar->rules[i].lhs;
            char Aj = grammar->rules[j].lhs;
            
            // Check if any production of Ai starts with Aj
...
void Direct_Left_recursion(Grammar* grammar, char* terminals) {
    ...
    // Remove indirect left recursion first, then direct
