...
void parseString(Grammar grammar,char *string,char* Terminals)
{
    char stack[100];
    int stack_top=0;
    stack[0]='$';
    stack[++stack_top] = grammar.rules[0].lhs;
    ...
    {
        input[++input_top]=string[i];
    }

    // for(int i=0;i<=input_top;i++)
    // {
    //     printf("%c ",input[i]);
    // }
    // printf("\n");
    
    while(stack_top>=0 && input_top>=0)
    {
        char stack_top_symbol = stack[stack_top];
        char input_top_symbol = input[input_top];

        if(input[input_top]=='$' && stack[stack_top]=='$')
        {
            printf("parsing successfull!!\n");
            return;
        }

        if (!(stack[stack_top]>='A' && stack[stack_top]<='Z'))
        {
            if (stack_top_symbol == input_top_symbol)
            {
                stack_top--;
                input_top--;
            }
            ...
            {
                printf("Error: Terminal mismatch (%c vs %c)\n", stack_top_symbol, input_top_symbol);
                return;
            }
        }
        else{
            for(int i=0;i<grammar.rule_count;i++)
            {
                if(grammar.rules[i].lhs==stack[stack_top])
                {
                    int var = getNonTerminalIndex(Terminals, input[input_top]);

                    int RHS=parsingTable[i][var];
                    if(RHS==-1)
                    {
                        printf("no entry in parsing table");
                        return;
                    }
                    char* temp2=grammar.rules[i].rhs[RHS];
                    stack_top--; 

                    if (strcmp(temp2, "e") != 0) {  
                        int len = strlen(temp2);
                        for (int j = len - 1; j >= 0; j--) {
                            stack[++stack_top] = temp2[j];
                        }
                    }
                }
            }
...
int main(int argc,char **argv)
{
   ...
   parseString(grammar,inputString,Terminals);
   return 0;
}
